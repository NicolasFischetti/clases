...
int abonado_init(Abonado* array,int limite)
{
    ...
    if(limite > 0 && array != NULL)
    {
        ...
        int limitesAbonadosPredefinidos = 10;

        for(i=0;i<limite;i++)
        {
            if(limitesAbonadosPredefinidos != -1 && limitesAbonadosPredefinidos > i) {
                array[i].idAbonado = i;
                strcpy(array[i].apellido, "Test");
                strcpy(array[i].nombre, "Test");
                strcpy(array[i].numero, "123DNI");
                array[i].isEmpty = 0;
            } else {
                array[i].isEmpty = 1;
            }
    ...
    if(limite > 0 && array != NULL)
    {
        ...
        if(indice >= 0)
        {
            ...
            if(!getValidString("Nombre?","Error","Overflow", nombre,50,2))
            {
                ...
                {
                    if(!getValidDni("Numero?","Error","Overflow",numero,2))
                    {
                        retorno = 0;
                        strcpy(array[indice].nombre,nombre);
                        strcpy(array[indice].apellido,apellido);
                        strcpy(array[indice].numero,numero);
        ...
        if(!getValidString("Nombre?","Error","Overflow", nombre,50,2))
            {
                if(!getValidString("Apellido?","Error","Overflow", apellido,50,2))
                     ...
                     if(!getValidDni("dni?","Error","Overflow",numero,2))
                    {
                {
                    retorno = 0;
                    strcpy(array[indice].nombre,nombre);
