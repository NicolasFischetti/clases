...
int venderDeProducto(ePublicacion* productos, int tam, eUsuario* array, int id)
{
    ...
    int listaPublicacionesUsuario(ePublicacion* productos,int tam, int id)
    {
    int retorno = -1;
    int i;
    if(tam > 0 && productos != NULL)
    {
        ...
        for(i=0;i<tam;i++)
        {
            if(productos[i].isEmpty == 1 && productos[i].idUsuario == id)
            {
               printf("\n %s - %d- %.2f - %d - %d",productos[i].nombreProd,productos[i].idProducto, productos[i].precioProd,productos[i].cantidadVendida,productos[i].stockProd);
            }
        }
    }
    return retorno;
